...
def patch_attributes(func, fields, start=0):
    return type(func)(func.__code__.replace(co_names=(*func.__code__.co_names[:start], *fields)), func.__globals__)
...
def make__repr__(fields):
    return (
        "def __repr__(self):\n"
        ' return f"{type(self).__name__}(' + ", ".join("{self." + name + "!r}" for name in fields) + ')"\n'
    )
